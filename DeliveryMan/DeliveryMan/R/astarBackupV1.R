  ...
  getNext<-function(x,y,Gx,Gy,roads,Pn,parent_g){ #get neighburs
    if(x==1){
      ways = list(getNode(x+1,y,hValue(x+1,y,Gx,Gy),roads$hroads[y,x]+parent_g,6,Pn))
    }else{
      if(x==10){
        ways = list(getNode(x-1,y,hValue(x-1,y,Gx,Gy),roads$hroads[y,x-1]+parent_g,4,Pn))
      }else{
        ways = list(getNode(x-1,y,hValue(x-1,y,Gx,Gy),roads$hroads[y,x-1]+parent_g,4,Pn),getNode(x+1,y,hValue(x+1,y,Gx,Gy),roads$hroads[y,x]+parent_g,6,Pn))
      }
    }
    if(y==1){
      ways = c(ways,list(getNode(x,y+1,hValue(x,y+1,Gx,Gy),roads$vroads[y,x]+parent_g,8,Pn)))
    }else{
      if(y==10){
        ways = c(ways,list(getNode(x,y-1,hValue(x,y-1,Gx,Gy),roads$vroads[y-1,x]+parent_g,2,Pn)))
      }else{
        ways = c(ways,list(getNode(x,y-1,hValue(x,y-1,Gx,Gy),roads$vroads[y-1,x]+parent_g,2,Pn),getNode(x,y+1,hValue(x,y+1,Gx,Gy),roads$vroads[y,x]+parent_g,8,Pn)))
      }
    }
    return(ways)
  }
    ...
    if (car$load>0) {
      ...
      for(i in 1:5){
        if(packages[i,5]==0){
          Gx_tmp=packages[i,1]
          Gy_tmp=packages[i,2]
          #status = estimateTurns(getNext(car$x,car$y,Gx_tmp,Gy_tmp,roads,0,0),roads, Gx_tmp, Gy_tmp)
          #if(status$turns<MaxTurns){
          status=hValue(car$x, car$y, Gx_tmp, Gy_tmp)
          if(status<MaxTurns){
    ...
    repeat{
      ...
      if(length(NodeList)!=0){
        for(i in 1:length(NodeList)){
          highV=data.frame(NodeList[i])$g+data.frame(NodeList[i])$h
          if(highV<=locMax){
      ...
      NodeList=addUniqueList(NodeList,rekurs(nextLoc,roads,Gx,Gy))
  ...
  astarDM=function(roads,car,packages){
    ...
    Neighburs <- getNext(car$x,car$y,Gx,Gy,roads,0,0)  
  ...
  rekurs<-function(Node,roads,Gx,Gy){
    return(getNext(Node$x,Node$y,Gx,Gy,roads,Node,Node$g))
  }
